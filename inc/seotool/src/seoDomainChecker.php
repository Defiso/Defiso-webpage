...
class seoDomainChecker{
 public $servers = array(
  ...
  public function GetAgeOfDomain($domain) {
    $return = array();
    $domain_parts = explode(".", $domain);
    $tld = strtolower(array_pop($domain_parts));
    $whoisserver = $this->servers[$tld];
    ...
    while(!feof($fp)){
      ...
      if(strpos(strtolower($out), "created:")) {
        $age = substr($out,strpos(strtolower($out), "created:"),strpos(strtolower($out), "created:")+2);
        $timestamp =  trim(substr($age, 9,19));
      }
      if(strpos(strtolower($out), "creation")){
        $age = substr($out, strpos(strtolower($out), "creation")+15, strpos(strtolower($out), "creation")+20);
        $month = intval(substr($age, 3,3));
        $year  = intval(substr($age, 9,7));
        $day  = intval(substr($age, 0,2));
        $month = date('m', strtotime($month));
        $timestamp = trim($year."-".$month."-".$day);
      }
    }
    fclose($fp);
    $current_time = time();
    $create_time  = strtotime(trim($timestamp));
    $time_diff = $current_time - $create_time;
    $day_diff = $time_diff / (3600 * 24);
    
    if (round($day_diff) > 365){
      $return["years"] = round($day_diff / 365);
      $return["days"]  = $day_diff % 365;
    }else{
       $return["days"]  = round($day_diff);
    }
    $return["timestamp"]  = $timestamp;
    $return["daydiff"] = round($day_diff);
    return $return;
  }
}